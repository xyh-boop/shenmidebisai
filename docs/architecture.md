...
JSON 是规范化结果，HTML 是由同一份已验证报告数据渲染出的展示层。仓库相对路径、稳定排序和基于内容的发现标识符，使离线扫描结果可以在多次运行之间直接比较。`AnalysisResult` 固定包含 `candidates`、`diagnostics` 和 `complete`；读取或分析任一影响覆盖完整性的错误都会让 `complete=false`，报告仍可生成但扫描退出码为 `3`。
...
- `providers` 对兼容 OpenAI API 的复核请求进行上下文裁剪、尽力过滤、发送与验证，同时执行请求数、令牌和费用预算限制。公网 endpoint 必须为 HTTPS；明文 HTTP 只在显式允许且目标为 loopback 时可用。
...
离线模式不会调用任何服务提供方。文件、候选、节点、决策和发现均采用稳定排序。模型辅助决策会单独记录请求数、令牌用量、预估成本、路由原因与延迟。每次提供方调用前均会检查硬性限制；本地预留值是不可退款硬预算，provider 自报 usage 仅作观测。

报告的最终严重性摘要只统计 `confirmed`、`likely` 和 `needs_review`。`rejected` 单列展示并附反证，避免把已拒绝候选混入最终发现数量。
